...
const ordersService = {
    getAllOrders: (page: string, config: IRequestConfig = {}, course_format?: string, course?: string, course_type?: string, status?: string, searchByName?: string,
                   searchBySurname?: string, searchByEmail?: string, searchByPhone?: string, searchByAge?: string, order?: string, orderBy?: string,)
        : IRes<IOrders> => {
        const params = {page, course_format, course, course_type, status, searchByName, searchBySurname, searchByEmail,
